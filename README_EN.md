...
Display AI provider quota, balance, and recent dialogue-round consumption in the pi TUI, with live quota-aware provider/model selection.
...
| `/aqauto` | `/aqauto [minutes]` | View/set idle auto refresh; `0` disables it |
| `/aqpick` | `/aqpick` | Refresh live quotas and choose a model by provider |

### `/aqcheck` — force refresh
...
- If a same-provider request is already in flight, the command waits for and reuses it; repeated `/aqcheck` calls for the same provider within 1 second are throttled;
...
### `/aqpick` — choose a model by live quota
...
- Takes no arguments and live-refreshes every candidate provider on each invocation; old snapshots are not used for the decision;
- Candidates come from the model scope currently allowed by pi; only authenticated providers supported by this extension are queried and shown;
- Candidate providers are queried concurrently with one live attempt each and a 10-second total deadline; failed, timed-out, or unauthenticated providers are omitted, and unselected candidate quota data stays in memory for that picker only instead of being cached locally;
- Percentage-based providers show **used percentage and reset countdown** for each window; balance providers show their balance;
- The current provider is pinned first and the rest are sorted by display name; choose a provider first, then one of its models; a provider with one selectable model switches directly;
- `Esc` cancels without changing the model; a failed switch leaves the current model unchanged;
- Switching uses pi's official `pi.setModel()` API, does not change authentication configuration, send a message, or trigger a model request;
- As with pi's native `/model`, after a cross-provider switch, **the next turn's conversation context is sent normally by pi to the new provider**.

![aqpick preview](https://raw.githubusercontent.com/Linen9/pi-check-agent-quota/main/assets/aqpick-en.png)
...
- Interval accepts integers only, in the range 0–30 minutes: `0` = off, `1–30` = enable and set the minute interval; `on`, `off`, decimals, negatives, multiple arguments, and values above 30 are rejected;
- Takes effect immediately and persists across sessions (stored in the local cache file, same as `/aqset`);
- Debounced: repeating the same `/aqauto` command within 1 second is ignored; any fetch attempt (manual `/aqcheck`, round settlement, session start) postpones the next auto fetch; in-flight requests are joined, never duplicated;
...
- Custom `baseUrl` allows only HTTPS (HTTP is allowed for loopback), and requests do not follow redirects. When configured, quota credentials are sent only to that address instead of the default official domain, so use trusted URLs only;
- By default the extension only fetches on user-visible events (session start, model switch, round settlement, `/aqcheck`). If you enable auto refresh via `/aqauto`, idle background fetches are sent on that interval while pi is open; `/aqpick` live-queries selectable, authenticated, supported providers when invoked;
- `/aqpick` uses pi's official model objects and `pi.setModel()` instead of constructing model requests; unselected candidate quota data is never persisted; next-turn context delivery after a cross-provider switch is identical to pi's native `/model`.
